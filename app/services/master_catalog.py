...
def _has_service():
    """
    У мастера должна существовать хотя бы одна
    валидная услуга с реально существующей
    категорией.
    """
    return exists(
        select(Service.id)
...
def _salon_access_condition():
    ...
    branch_belongs_to_salon = exists(
        select(Branch.id).where(
            Branch.id == Master.branch_id,
            Branch.salon_id == Master.salon_id,
        )
    )
...
def available_masters_query(
    *,
    ...
    Для мастера салона дополнительно проверяется,
    что выбранный филиал действительно принадлежит
    тому же salon_id.

    У каждой доступной услуги должна существовать
    связанная Category.
    """
    query = (
        select(Master)
