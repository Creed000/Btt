...
def can_add_branch(
    ...
    if current_count >= max_count:
        return (
            False,
            (
                f"На тарифе {get_plan_title(salon.plan)} "
                f"доступно не более {max_count} филиалов."
            ),
        )
...
def can_add_master(
    ...
    if current_count >= max_count:
        return (
            ...
            (
                f"На тарифе {get_plan_title(salon.plan)} "
                f"доступно не более {max_count} мастеров."
            ),
        )
...
def can_enable_master(
    ...
    if active_count >= max_count:
        return (
            ...
            (
                f"На тарифе {get_plan_title(salon.plan)} "
                f"одновременно могут принимать записи "
