...
async def _send_to_client(
    ...
    if not _notifications_allowed(client):
        logger.info(
            "Напоминание клиенту пропущено по настройкам. "
            "Запись: %s",
            booking.id,
        )
        return True
...
async def _send_to_master(
    ...
    if not _notifications_allowed(master_user):
        logger.info(
            "Напоминание мастеру пропущено по настройкам. "
            "Запись: %s",
            booking.id,
        )
        return True
...
async def process_booking_reminders(
    ...
    Настройка notifications_enabled учитывается отдельно
