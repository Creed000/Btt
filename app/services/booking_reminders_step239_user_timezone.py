...
def _reminder_texts(
    ...
    booking_datetime = _booking_datetime(
        booking
    )

    client_datetime_text = format_datetime_for_user(
        booking_datetime,
        client,
        pattern="%d.%m.%Y %H:%M",
    )

    master_datetime_text = format_datetime_for_user(
        booking_datetime,
        master_user,
        pattern="%d.%m.%Y %H:%M",
    )
    ...
    client_text = (
        ...
        f"Дата и время: {client_datetime_text}\n"
        f"Часовой пояс: "
        f"{client.timezone if client and client.timezone else 'по умолчанию'}"
    )
    ...
    master_text = (
        ...
        f"Услуга: {service_title}\n"
        f"Дата и время: {master_datetime_text}\n"
        f"Часовой пояс: "
        f"{master_user.timezone if master_user and master_user.timezone else 'по умолчанию'}"
    )
