...
def _lock_master_booking_day(
    db,
    master_id: int,
    booking_date: date,
) -> None:
    """
    Сериализует создание записей одного мастера на одну дату.

    PostgreSQL advisory lock действует только до завершения текущей
    транзакции. Для SQLite функция ничего не делает.
    """
    bind = db.get_bind()
...
def create_booking(
    ...
    try:
        ...
        # Не даём двум параллельным запросам одновременно пройти
        # проверку свободного времени одного мастера на одну дату.
        _lock_master_booking_day(
            db,
            master_id,
            booking_date,
        )
