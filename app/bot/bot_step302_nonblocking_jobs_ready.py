...
async def subscription_maintenance_job(
    ...
    """
    try:
        # SQLAlchemy здесь синхронный. Не блокируем
        # event loop Telegram-бота на больших таблицах.
...
async def master_invite_maintenance_job(
    ...
    try:
        # Фоновая очистка использует синхронную БД,
        # поэтому выполняем её вне event loop.
        await asyncio.to_thread(
            expire_stale_master_invites
        )
    except Exception:
