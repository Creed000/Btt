...
from app.handlers.settings_notifications import (
    change_user_notifications,
    open_notification_settings,
)
from app.handlers.settings_panel import open_settings_panel
from app.handlers.settings_timezone import (
...
from app.handlers.help_panel import show_help
...
from app.services.plan_catalog import resolve_plan_code
...
def clear_booking_data(
    ...
    for key in (
        ...
        "booking_date",
        "booking_time",
        "booking_confirmation_processing",
    ):
...
async def cancel_client_booking(
    ...
    try:
        ...
        # использует User FOR UPDATE. Берём тот же row-lock
        # до Booking FOR UPDATE, чтобы отмена не прошла
        # одновременно с отключением клиентского аккаунта.
        user = db.scalar(
            select(User)
            .where(
        ...
        if not user.is_active:
            await show_main_menu(
                update,
                context,
                text="❌ Ваш аккаунт временно отключён.",
            )
            return
        ...
        if booking_item is None:
            await show_main_menu(
                ...
                context,
                text=(
                    "❌ Запись не найдена или принадлежит "
        ...
        if booking_item.status == "completed":
            await show_main_menu(
                update,
                context,
                text="❌ Завершённую запись отменить нельзя.",
            )
            return
        ...
        if starts_at <= local_naive_now():
            await show_main_menu(
                update,
                context,
                text="❌ Прошедшую запись отменить нельзя.",
            )
            return
...
async def menu(
    ...
    timezone_handled = await change_user_timezone(
        update,
        context,
    )
    if timezone_handled:
    ...
    notifications_handled = await change_user_notifications(
        update,
        context,
    )
    if notifications_handled:
        return
    ...
    if text in {"💼 Стать мастером", "💼 Кабинет мастера"}:
    ...
    if text == "🔔 Уведомления":
        await open_notification_settings(
            update,
            context,
        )
        return
    ...
    if text.startswith("🏙 "):
        ...
        if selected_city is None:
            await update.message.reply_text(
                "❌ Этот город больше недоступен. Выберите город заново.",
                reply_markup=main_menu(
                    telegram_id=update.effective_user.id,
                ),
            )
    ...
    if text.startswith("💼") and "#" in text:
        ...
        if (
            ...
            clear_booking_data(context)
            await update.message.reply_text(
                "⚠️ Данные выбора потеряны. Начните запись заново.",
                reply_markup=main_menu(
                    telegram_id=update.effective_user.id,
                ),
            )
    ...
    if text.startswith("🕒 "):
        ...
        if (
            ...
            clear_booking_data(context)
            await update.message.reply_text(
                "⚠️ Данные записи потеряны. Начните запись заново.",
                reply_markup=main_menu(
                    telegram_id=update.effective_user.id,
                ),
            )
        ...
        master_name = context.user_data.get(
            "master_name",
            "Мастер",
        )
    ...
    if text == "✅ Подтвердить":
        if context.user_data.get(
            ...
            await update.message.reply_text(
                "⏳ Запись уже обрабатывается. "
                "Не нажимайте подтверждение повторно."
            )
            return
        ...
        if not all(
            ...
            await show_main_menu(
                ...
                context,
                text=(
                    "⚠️ Данные записи заполнены не полностью. "
                    "Начните запись заново."
                ),
            )
            return
        ...
        # Запись уже создана в БД.
        # Очищаем состояние ДО любых await, чтобы повторное
        # нажатие не могло повторно использовать старые данные.
        clear_booking_data(context)
