...
def lock_master_day(
    ...
    """
    Сериализует изменение доступности мастера на конкретную дату.

    Ключ намеренно совпадает с create_booking(), чтобы операция
    "поставить выходной" не могла пересечься с созданием новой записи.
    PostgreSQL advisory lock живёт до commit/rollback.
    """
    bind = db.get_bind()
...
async def process_day_off_creation(
    ...
    try:
        ...
        # Один и тот же advisory lock используется и при create_booking().
        # Пока мастер ставит выходной, новая запись на эту дату ждёт.
        lock_master_day(
            db,
            master.id,
            selected_date,
...
async def delete_master_day_off(
    ...
    try:
        ...
        # Снятие выходного снова открывает дату для записи,
        # поэтому синхронизируем его с create_booking().
        lock_master_day(
            db,
            master.id,
            selected_date,
