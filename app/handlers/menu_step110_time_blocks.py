...
async def menu(
    ...
    # Продолжаем незавершённое добавление блокировки времени мастера.
    if context.user_data.get("time_block_setup"):
        handled = await process_time_block_creation(
            update,
            context,
        )
        if handled:
    ...
    # Продолжаем незавершённое добавление отдельного выходного мастера.
    if context.user_data.get("day_off_setup"):
        handled = await process_day_off_creation(
            update,
            context,
        )
        if handled:
    ...
    # Продолжаем незавершённую настройку расписания мастера.
    if context.user_data.get("schedule_setup"):
        handled = await process_schedule_setup(
            update,
            context,
        )
        if handled:
    ...
    # Продолжаем незавершённое назначение филиала мастеру.
    if context.user_data.get("branch_assignment"):
        handled = await process_branch_assignment(
            update,
            context,
        )
        if handled:
    ...
    # Продолжаем незавершённое приглашение мастера раньше остальных меню.
    if context.user_data.get("master_invite"):
        handled = await process_master_invite(
            update,
            context,
        )
        if handled:
    ...
    # Продолжаем незавершённое создание филиала раньше остальных меню.
    if context.user_data.get("branch_creation"):
        handled = await process_branch_creation(
            update,
            context,
        )
        if handled:
    ...
    # Продолжаем незавершённую регистрацию салона раньше всех меню.
    if context.user_data.get("salon_registration"):
        handled = await process_salon_registration(
            update,
            context,
        )
        if handled:
    ...
    # Продолжаем выбор города мастера раньше обычной обработки городов.
    if context.user_data.get("master_city_selection"):
        handled = await process_master_city_selection(
            update,
            context,
        )
        if handled:
    ...
    if text == "💳 Подписки салонов":
        await show_salons_for_subscription(
            update,
            context,
        )
        return
    ...
    # Кабинет мастера: отдельные выходные даты.
    ...
    # Кабинет мастера: блокировки времени.
    ...
    # Запись клиента: дата из динамического календаря.
    if text.startswith("📅 ") and "·" in text:
        try:
    ...
    if text == "✅ Подтвердить":
        ...
        # Уведомляем мастера. Ошибка отправки не отменяет запись.
