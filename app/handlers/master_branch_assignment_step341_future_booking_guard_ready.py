...
async def process_branch_assignment(
    ...
    if stage == "branch":
        ...
        try:
            ...
            # После Master FOR UPDATE проверяем будущие записи.
            # Booking пока не хранит отдельный snapshot филиала,
            # поэтому перенос мастера с будущими записями
            # сделал бы место оказания услуги неоднозначным.
            if has_future_active_bookings(
                db,
                master.id,
            ):
                raise ValueError(
                    "Нельзя менять филиал мастера, пока есть "
                    "будущие активные записи. Сначала завершите, "
