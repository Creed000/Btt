...
def lock_master_day(
    ...
    """
    Сериализует изменения доступности одного мастера на одну дату.

    Ключ совпадает с create_booking() и master_day_off_manager,
    поэтому запись, выходной и блокировка времени не могут
    конкурентно пройти проверки на одной дате.
    """
    bind = db.get_bind()
...
async def process_time_block_creation(
    ...
    if stage == "end":
        ...
        try:
            ...
            # Тот же advisory lock используется при create_booking()
            # и добавлении выходного. После получения блокировки
            # заново читаем расписание, блокировки и записи.
            lock_master_day(
                db,
                master.id,
                block_date,
...
async def delete_master_time_block(
    ...
    try:
        ...
        # Удаление тоже меняет доступность даты.
        # Используем тот же lock, что create_booking() и создание блока.
        lock_master_day(
            db,
            master.id,
            block_date,
        ...
        # После ожидания блокировки перечитываем строку:
        # другой процесс мог уже удалить её.
        block = db.scalar(
            select(MasterTimeBlock).where(
                MasterTimeBlock.id == block_id,
