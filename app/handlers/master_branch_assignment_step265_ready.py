...
async def begin_branch_assignment(
    ...
    try:
        ...
        if salon is None:
            await update.message.reply_text(
                "⛔ Назначать филиалы может только "
...
async def process_branch_assignment(
    ...
    if stage == "branch":
        ...
        try:
            ...
            if master is None:
                raise ValueError(
                    "Мастер не найден или больше не принадлежит вашему салону."
                )
            ...
            if master.branch_id == branch.id:
                raise ValueError(
                    "Этот филиал уже назначен выбранному мастеру."
                )
        ...
        if master_telegram_id is not None:
            try:
                await context.bot.send_message(
                    ...
                    text=(
                        "🏬 Вам назначили филиал в GlowFlow.\n\n"
                        f"Филиал: {branch_name}"
                    ),
                )
            except Exception:
