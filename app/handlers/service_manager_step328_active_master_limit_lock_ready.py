...
async def toggle_master_booking(
    ...
    try:
        ...
        if enabled:
            # Для мастера салона сериализуем включение записи
            # по одной строке Salon. Иначе два мастера могут
            # одновременно пройти can_enable_master() и вместе
            # превысить лимит активных мастеров тарифа.
            if master.salon_id is not None:
                ...
                # Подменяем relationship актуальной
                # заблокированной строкой перед readiness.
