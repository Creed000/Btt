...
async def approve_plan_request(
    ...
    try:
        ...
        salon = db.scalar(
            select(Salon)
            .where(
                Salon.id == request.salon_id
            )
            .with_for_update()
        )

        # После получения блокировки салона повторно блокируем
        # и перечитываем сам запрос: его мог обработать другой админ.
        request = db.scalar(
            select(SubscriptionPlanRequest)
            .options(
                joinedload(
        ...
        if salon is None:
            ...
            await update.message.reply_text(
                "❌ Салон запроса больше не существует. "
                "Запрос закрыт.",
                reply_markup=admin_menu(),
            )
            return
